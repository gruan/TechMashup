...
/*
* allows the user to rename a class
*/
function renameClass(){
	var toChange = prompt("What is the class that you would like to rename");

	if(classes[0][0] === "Generic Class"){
		alert("Add a class first!");
	}

	for(var i = 0; i < classes.length; i++){
		if(toChange === classes[i][0]){
			var cname = prompt("What should be the new name of " + toChange + "?");
			document.getElementById("p"+(i+1)).innerHTML = cname;
			classes[i][0] = cname;
		}
	}
	save();
...
/*
* allows the user to rename a link 
*/
function renameLink(){
	var linkName = prompt("Enter the name of your link");
	for(var i = 0; i < localStorage.length; i++){
		if(localStorage.getItem(localStorage.key(i)) === linkName){
			var link = localStorage.key(i); //the actual link
			//find the link in classes and change the value
			for(var j = 0; j < classes.length; j++){
				for(var z = 0; z < classes[j].length; z++){
					if(classes[j][z] === link){
						var newLink = prompt("What should the new link be?");
						if(newLink != null){
							classes[j][z] = newLink;
							var linkElement = document.getElementById("l"+(j+1)+z);
							linkElement.href = newLink;
							var newName = prompt("What would you like to call this link?");
							if(newName != null){
